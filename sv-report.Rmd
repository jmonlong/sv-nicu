...
## args = c('svs_tier1.tsv', 'svs_tier1_finetuned.vcf', 'svs_other.tsv', 'indexcov_res/indexcov_res-indexcov.bed.gz', 'sv_annotation_database.RData', 'out.csv')
...
svs = rbind(svs.t1 %>% select(seqnames, start, end, svid, type, size, qual, freq, ac),
            svs.o %>% select(seqnames, start, end, svid, type, size, qual, freq, ac))
...
svs.gene %>% as.data.frame %>% mutate(chr=seqnames, pos=start) %>%
  filter(gene_type=='protein_coding', freq<.01, impact=='coding') %>%
  merge(pli.df, all.x=TRUE) %>%
  mutate(pLI=ifelse(is.na(pLI), -1, pLI)) %>% 
  merge(genes.list.s, all.x=TRUE) %>% 
  select(gene_list, gene, pLI,
         pos, svid, type, size, large, qual, ac, freq,
         simp.rep, dgv, cons, cres,
         chr, end) %>%
  arrange(is.na(gene_list), large, desc(pLI), freq, desc(size)) %>%
  formatTable() %>%
  datatable(filter='top', escape=FALSE, options=list(pageLength=25))
```
...
svs.gene %>% as.data.frame %>% mutate(chr=seqnames, pos=start) %>%
  filter(gene_type=='protein_coding',
         freq<.01, impact %in% c('UTR', 'promoter')) %>%
  merge(pli.df, all.x=TRUE) %>%
  mutate(pLI=ifelse(is.na(pLI), -1, pLI)) %>% 
  merge(genes.list.s, all.x=TRUE) %>% 
  select(gene_list, gene, impact, pLI,
         pos, type, size, large, qual, ac, freq,
         simp.rep, dgv, cons, cres,
         chr, end) %>%
...
svs.gr %>% as.data.frame %>% mutate(chr=seqnames, pos=start) %>%
  filter(sel.gene.dist>0, sel.gene.dist<1e5, freq<.01) %>%
  select(sel.gene.dist, sel.gene.near, 
         pos, type, size, large, qual, ac, freq, genes,
...
```{r table_noncoding}
svs.gr %>% as.data.frame %>% mutate(chr=seqnames, pos=start) %>%
  filter(freq<.01, cres | cons | ctcf,
         gene.dist>0) %>%
  select(pos, type, size, large, qual, ac, freq, genes,
         cons, cres, ctcf,
         gene.dist, gene.near,
         simp.rep, dgv, 
         chr, end) %>%
...
```{r rarelarge}
svs.gr %>% as.data.frame %>% mutate(chr=seqnames, pos=start) %>%
  filter(size>1e3, freq<.01) %>%
  select(pos, type, size, qual, ac, freq, 
         nb.pc.genes, genes,
